    ...
    def generate_questions(self, instance):
        try:
            ...
            for i in range(half_num):
                ...
                self.right_layout.add_widget(Label(text=f"{i + 1}. Öğrenci: {left_question}",color=(1, 1, 0, 1),font_size='25sp'))
                if right_question:
                    self.right_layout.add_widget(Label(text=f"{right_index + 1}. Öğrenci: {right_question}",color=(1, 1, 0 , 1),font_size='25sp'))
